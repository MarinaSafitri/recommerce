...
//checkshop ==============================
app.get('/wooapi/updatedata',function(req,res){
  ...
  .then(function(result){
      ...
      .then(function(result){
        ...
        .then(function(result){
          ...
          session.close();
       res.redirect('/wooapi/getdatacustsmw?cs='+shopArr[0]['cs']+'&ck='+shopArr[0]['ck']+'&url='+shopArr[0]['urlshop']+'&token='+token+'&idcust='+idcust+'&tp='+shopArr[0]['totalproduk'].low+'&tc='+shopArr[0]['totalcust'].low+'&to='+shopArr[0]['totalorder'].low)
          })
...
//awal======================
app.get('/wooapi/getdatacustsmw',function (req,res){
...
function addCustomer(WooCommerce,file,req, res){
  ...
  session
    ...
    .then(function(result){
    res.redirect('/wooapi/getdataproductssmw?cs='+req.query.cs+'&ck='+req.query.ck+'&url='+req.query.urlshop+'&token='+req.query.token+'&tp='+req.query.tp+'&tc='+req.query.tc+'&to='+req.query.to)
    session.close();
  })
    .catch(function(err){
    console.log(err);
...
app.get('/wooapi/getdataproductssmw',function (req,res){
...
function addProducts(WooCommerce,file,req, res){
  var token = req.query.token;
  session
  .run('WITH "http://localhost/ta/nodemovies/'+file+'.json" AS url CALL apoc.load.json(url) YIELD value UNWIND value.products AS p MERGE (product:Products {idprod:p.id}) ON CREATE SET product.name = p.name, product.token={tokenParam}',{tokenParam:token})
...
app.get('/wooapi/getdataordersmw',function (req,res){
